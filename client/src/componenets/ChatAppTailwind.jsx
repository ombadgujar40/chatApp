      ...
      <div className="w-full max-w-5xl h-[640px] rounded-xl overflow-hidden grid grid-cols-[320px_1fr] shadow-2xl bg-cardbg-40">
        ...
        <aside className="p-4 bg-transparent flex flex-col gap-4 border-r border-white-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-lg bg-avatar-gradient flex items-center justify-center font-semibold text-slate-900">
                ME
              </div>
              <div>
            ...
            <div>
              <button
                ...
                className="cursor-pointer text-xs px-2 py-1 border border-sky-400 text-sky-400 rounded-lg hover:opacity-90 disabled:opacity-50"
              >
                Log Out
              </button>
            </div>
          ...
          {/* <SearchBar value={search} name="search" onChange={(e) => { setSearch(e.target.value) }} /> */}
          <ChatListStatic
            users={users}
            me={user}
            onChatCreated={fetchUsers}
            msg={messages}
          />
        </aside>
        ...
        <main className="flex flex-col">
          ...
          <div className="p-3 border-t border-white/5 flex gap-3 items-center">
            ...
            />
            <button
              disabled={!text || !selectedChat}
              onClick={sendMessage}
              className="px-4 py-2 bg-accent text-slate-900 rounded-lg font-semibold opacity-40"
            >
              Send
            </button>
          </div>
