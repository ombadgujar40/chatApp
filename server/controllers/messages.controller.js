...
exports.createMessage = asyncHandler(async (req, res) => {
  ...
  if (!chatId || !text) return res.status(400).json({ ok: false, msg: 'chatId and text required' });

  const msg = await Message.create({ chatId, sender, text, status: 'delivered' });

  // update chat lastMessageId
  await Chat.findByIdAndUpdate(chatId, { lastMessageId: msg._id });

  const populated = await Message.findById(msg._id).populate('sender', 'name email').lean();
  res.json({ ok: true, message: populated });
});

exports.getMessagesForChat = asyncHandler(async (req, res) => {
  const { chatId } = req.params;
  const messages = await Message.find({ chatId }).populate('sender', 'name email').sort({ createdAt: 1 }).lean();
